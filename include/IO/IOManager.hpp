...
public:
    ...
    unsigned give(IOComponent* pComponent);//we recieve a pointer to a component and we store it and remember the name and position
    void free(unsigned position);//don't adjust the list, just mark the node as null and offer it as a position to future customers
...
private:
    ...
    std::vector<unsigned> m_freeIndexes;//spots where the ptr is null and we can give a position out!
    std::map<std::string, unsigned> m_nameLookup;//where we lookup the position of an IOComponent after we are told its name
};
